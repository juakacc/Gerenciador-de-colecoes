...
public class GerenciaItem implements GerenciamentoIF {
	...
	public void selecionar(int opcao) {
		...
		case 2:		// editar item
			try {
				item = recuperarItemAll();
				System.out.println("Se desejar manter a característica antiga press enter no campo"
						+ "\nNovas informações: ");
				
				if (item instanceof DvdCd) {
					FabricaItem.editarDvdCd((DvdCd) item);
				} else if (item instanceof Hq) {
					FabricaItem.editarHq((Hq) item);
				} else if (item instanceof JogoDeTabuleiro) {
					FabricaItem.editarJogoDeTabuleiro((JogoDeTabuleiro) item);
				} else if (item instanceof JogoDeVideoGame) {
					FabricaItem.editarJogoDeVideoGame((JogoDeVideoGame) item);
				}
