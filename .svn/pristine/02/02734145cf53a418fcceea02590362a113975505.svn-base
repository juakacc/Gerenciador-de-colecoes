...
public class GerenciaDesejos implements GerenciamentoIF {
	...
	/**
	 * Gerencia a remoção dos itens cadastrados.
	 * @param opcao - Opção para a remoção
	 * */
	private void removerItem(int opcao) {
		if (opcao == 1) {
			try {
				Item item = recuperarItemDesejado();
				if (meusDesejos.removerItem(item)) {
			...
			if (meusDesejos.removerTodos()) {
				System.out.println("\nTodos os desejos foram removidos...");
			}
		}
	}
	...
	private JogoDeVideoGame jogoVDesejado() {
		...
		return 	new JogoDeVideoGame(titulo, "novo", preco, " ", 5, console, false);
	}
	...
	private Hq hqDesejado() {
		...
		return new Hq(titulo, "novo", preco, " ", 5, numero, editora, universo, false);
	}
	
	/**
	 * Altera os atributos depois que o mesmo é adquirido caso o usuário
	 * deseje antes de adicionar ao se Acervo.
	 * @param item - Item que foi adquirido
	 * */
	public static void alterarAtributosRestantes(Item item) {
